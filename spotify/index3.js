...
//to add play and pause in the playbar
...
async function getAlbums() {
  ...
  Array.from(document.querySelectorAll(".card")).forEach((element) => {
    element.addEventListener("click", async (e) => {
      // console.log(element);
      let albumName = element.querySelector("h2").innerHTML;
     ...
     let  songs = await getSongs2(element.querySelector("h2").innerHTML);
      ...
      for (const song of songs) {
       ...
       let songName = ((((song.split("/albums")[1]).replaceAll("%20"," ")).split(`/${albumName}/`))[1]).split(".mp3")[0];
...
async function main() {
  ...
  Array.from(
    ...
    e.addEventListener("click", (element) => {
      console.log(e.querySelector(".song-info").getElementsByTagName("div")[0].innerHTML)
...
// async function rand(){
//   let songDetails = await fetch("http://127.0.0.1:5500/spotify/songs");
//   let response = await songDetails.json();
